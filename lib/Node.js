...
class Node extends Model {
	 ...
	 * @returns {Relationship} The Relastionship class
	 */
	 ...
	 *
	 * @param {Array} [relationships] The relationships to be saved (all by default)
	 * @param {Transaction} [tx] The current databse transaction
	 */
	async saveRelated(relationships = null, tx = null)
	{
		relationships = relationships || Object.keys(this.constructor.relationships)
	...
	async addLabels(labels, tx = null)
	{
		...
		await tx.run(query, paramaters).then(r => r)
	...
	static parseQueryFilters(filters, variable)
	{
		...
		{
			throw `Invalid filter object: ${ filters }`
		}
	...
	static async merge(criteria, properties, o = {})
	{
		...
		if (keys.length)
		{
			...
			{
				paramaters.push(`${ key }: {criteria}.${ key }`)
			}
