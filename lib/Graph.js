...
class Graph {
	...
	getRelated(node, $Relationship)
	{
		...
		for (const [, item] of this.relationships)
		{
			...
			// TODO: check relationships with direction 'both'

			if ([Graph.Relationship.OUT, Graph.Relationship.ANY].includes($Relationship.direction) && relationship.start.equals(entity.identity))
			{
				other = this.getNodeModel(relationship.end, $Relationship.Model)
			}
			else if ([Graph.Relationship.IN, Graph.Relationship.ANY, Graph.Relationship.BOTH].includes($Relationship.direction) && relationship.end.equals(entity.identity))
			{
